...
/*
	...
	The total length of each bit is 1.25�s (15 cycles @ 12Mhz)
	* At 0�s the dataline is pulled high.  (cycle 1+0)
	* To send a zero the dataline is pulled low after 0.333�s (1+4=5 cycles).
	* To send a one the dataline is pulled low after 0.666�s (1+8=9 cycles).

	Total loop timing is correct, but the timing for the falling edge can
	not be accurately reached as the correct 0.375�s (4.5 cyc.) and 0.675�s (7.5 cyc)
	timings fall in between cycles.
	
	Final timing: 
	* 15 cycles for bits 7-1
	* 16 cycles for bit 0	
	- The bit 0 timing exceeds the 1.25�s bit-timing by 66.7�s, which is still
	within datasheet tolerances (600�s)
*/
...
/*
	...
	The total length of each bit is 1.25�s (10 cycles @ 8Mhz)
	* At 0�s the dataline is pulled high.  (cycle 1+0=1)
	* To send a zero the dataline is pulled low after 0.375�s (1+3=4 cycles).
	* To send a one the dataline is pulled low after 0.625�s (1+5=6 cycles).

	Final timing: 
	* 10 cycles for bits 7-1
	* 14 cycles for bit 0		
	
	- The bit 0 timing exceeds the 1.25�s bit-timing by 500�s, which is still
	  within datasheet tolerances (600�s)
*/
...
The total length of each bit is 1.25�s (12 cycles @ 9.6Mhz)
* At 0�s the dataline is pulled high.  (cycle 1)
* To send a zero the dataline is pulled low after 0.312�s (1+3=4 cycles) (error 0.06�s)
* To send a one the dataline is pulled low after 0.625�s (1+6=7 cycles) (no error).
...
* 15 cycles for bit 0
	...
	- The bit 0 timing exceeds the 1.25�s timing by 312�s, which is still within
	  datasheet tolerances (600�s).
*/
...
/*
	...
	The total length of each bit is 1.25�s (5 cycles @ 4Mhz)
	* At 0�s the dataline is pulled high.  (cycle 0+1)
	* To send a zero the dataline is pulled low after 0.5�s (spec: 0.375�s)  (2+1=3 cycles).
	* To send a one the dataline is pulled low after 0.75�s  (spec: 0.625�s) (3+1=4 cycles).
	...
	* 6 cycles for bit 0	
	...
	- The bit 0 timing exceeds the 1.25�s timing by 250�s, which is still within
	the tolerances stated in the datasheet (600 �s).
